...
cpu_manager::cpu_manager(long batch_size, int dimension, double *host_database_B,
    ...
    this->result_vector = (double *) malloc(batch_size * sizeof(double));
...
void cpu_manager::search() {
    ...
    for (int k = 0; k < batch_size; k++) {
        ...
        result_vector[k] = 0.00;
        for (int i = 0; i < dimension; i++) {
            result_vector[k] += pow(host_input_B[i], 2) / (4 * host_input_A[0]); // additive ab1 1st item
            result_vector[k] +=
                    pow(host_database_B[offset + i], 2) / (4 * host_database_A[k]); // additive ab2 1st item
            result_vector[k] -= pow(host_database_B[offset + i] + host_input_B[i], 2) /
                                (4 * (host_database_A[k] + host_input_A[0])); // subtractive ab_star 1st item
        }
        result_vector[k] += 0.5 * dimension * log(-1 * (host_input_A[0]) / M_PI); // additive ab1 2nd item
        result_vector[k] += 0.5 * dimension * log(-1 * (host_database_A[k]) / M_PI); // additive ab2_2nd item
        result_vector[k] -= 0.5 * dimension * log(-1 * (host_database_A[k] + host_input_A[0]) /
                                                  M_PI); // subtractive ab_star 2nd item
        result_vector[k] -= 0.5 * dimension * log(2 * M_PI); // subtractive cons
        result_vector[k] += host_database_probY[k];
    }
