...
gpu_manager::gpu_manager(long data_size, int dimension){
    ...
    this->device_data_size = data_size / this->num_devices;

    std::cout << "========================Initializing data for all GPUs======================================\n" << std::endl;
    for(int i=0; i<this->num_devices; i++)
        this->list_of_users.push_back(new single_gpu_manager(i, device_data_size, dimension));
...
//    list_of_users.at(device_id) = new single_gpu_manager(device_id, device_data_size, dimension);
//}
...
void gpu_manager::copy_database_to_device(double* host_database_B, double* host_database_A, double* host_database_prob_Y) {
    ...
    for(int i=0; i<list_of_users.size(); i++){
        long offset = compute_device_data_offset(i);
...
void gpu_manager::search() {
    ...
    double gflops = calculate_gflops(time_sec, device_data_size * dimension);
    print_on_screen("GPU Search", time_sec, gflops, device_data_size * dimension, 1);

}


long gpu_manager::compute_device_data_offset(int batch_id){
    return batch_id * device_data_size;
}
